...
console
    ...
    for(let fireball of fireballs) {
        ...
        if(gameState == "action") {
            ...
            { // if they do, end the game
                ...
                gameState = "gameover";
                alert("Game Over");
                if(confirm("continue playing" + username +"?")) {
                    gamestate = "action";
                    fireballs = fireballs.remove(fireball)
                }
            }
    ...
    if(gameState == "action") {
        ...
        if(diamondXPosition < destructionXPosition) {
            diamond = diamond.remove(diamond);
        }
...
function updateSpeed() {
    ...
    for(let speed of speeds) {
        speed.xPosition += speedXSpeed;
 
        if(gameState == "action") {
            // check if the speed collides with the bird
            if(theseCirclesCollide(
                birdXPosition,
                birdYPosition,
                birdHitboxRadius,
            ...
            { // if they do, increase the speed
                coinSound.play();
                fireballXSpeed *= 2,
                coinXSpeed *= 2,
                diamondXSpeed *= 2,
                speeds = speeds.remove(speed);
            }
 
            if(speedXPosition < destructionXPosition) {
                speeds = speeds.remove(speed);
            }
        }
    ...
    // spawn new speeds
    if(gameState == "action" &&
    speedTimeSinceLastSpawn>speedSpawnInterval) {
        speeds.push({
            xPosition: spawnXPosition,
            yPosition: randomBetween(0, canvas.height)
        });
        speedTimeSinceLastSpawn = 0;
    }
}
